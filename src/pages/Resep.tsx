...
function StatusBadge({ status }: { status: Prescription['status'] }) {
  const map = {
    ...
    cancelled: 'bg-gray-100 dark:bg-zinc-800 text-gray-500 dark:text-gray-400',
  } as const;
    ...
    <div className="flex-1 pb-20 lg:pb-0">
      ...
      <main className="p-6 lg:p-8 max-w-[1400px] mx-auto w-full">
        ...
        <div className="bg-white dark:bg-zinc-900 rounded-xl border border-gray-200 dark:border-zinc-700 shadow-sm overflow-hidden">
          {loading ? (
            <div className="p-10 text-center text-gray-400 dark:text-gray-500 text-sm">Memuat data...</div>
          ) : filtered.length === 0 ? (
            <div className="p-10 text-center">
              <ClipboardText weight="fill" className="w-10 h-10 text-gray-200 mx-auto mb-3" />
              <p className="text-gray-400 dark:text-gray-500 text-sm">Belum ada resep</p>
            </div>
          ) : (
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-100 dark:border-zinc-800 bg-gray-50/70 dark:bg-zinc-800/70">
                  <th className="text-left py-3 px-4 font-semibold text-gray-500 dark:text-gray-400 text-xs uppercase tracking-wide">No. Resep</th>
                  <th className="text-left py-3 px-4 font-semibold text-gray-500 dark:text-gray-400 text-xs uppercase tracking-wide">Pasien</th>
                  <th className="text-left py-3 px-4 font-semibold text-gray-500 dark:text-gray-400 text-xs uppercase tracking-wide hidden md:table-cell">Dokter</th>
                  <th className="text-left py-3 px-4 font-semibold text-gray-500 dark:text-gray-400 text-xs uppercase tracking-wide hidden sm:table-cell">Tgl Resep</th>
                  <th className="text-left py-3 px-4 font-semibold text-gray-500 dark:text-gray-400 text-xs uppercase tracking-wide">Status</th>
              ...
              <tbody>
                {filtered.map(resep => (
                  ...
                  >
                    <td className="py-3 px-4 font-mono text-xs text-gray-600 dark:text-gray-300">{resep.prescription_number}</td>
                    <td className="py-3 px-4 font-medium text-gray-900 dark:text-gray-100">
                      {resep.patient_name}
                      {resep.patient_age ? <span className="text-gray-400 dark:text-gray-500 font-normal"> · {resep.patient_age} thn</span> : null}
                    </td>
                    <td className="py-3 px-4 text-gray-600 dark:text-gray-300 hidden md:table-cell">dr. {resep.doctor_name}</td>
                        ...
                        <button
                          ...
                          className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold ml-auto ${
                            resep.valid_until && new Date(resep.valid_until) < new Date()
                              ? 'bg-zinc-200 dark:bg-zinc-700 text-gray-400 dark:text-gray-500 cursor-not-allowed'
          ...
          <div className="bg-white dark:bg-zinc-900 rounded-xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto animate-in fade-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between p-5 border-b border-gray-100 dark:border-zinc-800">
              <div>
                ...
                <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5 font-mono">{selected.prescription_number}</p>
              </div>
            ...
            <div className="p-5 space-y-4">
              <div className="grid grid-cols-2 gap-3 text-sm">
                <div>
                  <p className="text-gray-400 dark:text-gray-500 text-xs mb-0.5">Pasien</p>
                  <p className="font-semibold">{selected.patient_name}{selected.patient_age ? `, ${selected.patient_age} thn` : ''}</p>
                </div>
                <div>
                  <p className="text-gray-400 dark:text-gray-500 text-xs mb-0.5">Dokter</p>
                  <p className="font-semibold">dr. {selected.doctor_name}</p>
                  {selected.doctor_sip && <p className="text-xs text-gray-400 dark:text-gray-500">SIP: {selected.doctor_sip}</p>}
                </div>
                <div>
                  <p className="text-gray-400 dark:text-gray-500 text-xs mb-0.5">Tgl Resep</p>
                ...
                <div>
                  <p className="text-gray-400 dark:text-gray-500 text-xs mb-0.5">Status</p>
                  <StatusBadge status={selected.status} />
                </div>
                {selected.valid_until && (
                  <div className="col-span-2">
                    <p className="text-gray-400 dark:text-gray-500 text-xs mb-0.5">Berlaku Sampai</p>
                    <p className={`font-semibold text-sm ${new Date(selected.valid_until) < new Date() ? 'text-rose-600' : 'text-gray-900 dark:text-gray-100'}`}>
              ...
              {selected.prescription_items && selected.prescription_items.length > 0 && (
                <div>
                  <p className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-2">Obat yang Diresepkan</p>
                  <div className="space-y-2">
                    {selected.prescription_items.map((item: PrescriptionItem) => (
                      <div key={item.id} className="flex items-start justify-between bg-gray-50 dark:bg-zinc-800 rounded-xl px-3 py-2.5">
                        <div>
                          <p className="font-semibold text-sm">{item.medicine_name}</p>
                          {item.signa && <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">{item.signa}</p>}
                        </div>
                        <p className="text-sm font-semibold text-gray-700 dark:text-gray-200 ml-4 shrink-0">{item.quantity}x</p>
                      </div>
                    ))}
                  </div>
                </div>
            ...
            <div className="flex gap-2 p-5 border-t border-gray-100 dark:border-zinc-800 flex-wrap">
        ...
        <div className="fixed inset-0 bg-zinc-900/50 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white dark:bg-zinc-900 rounded-xl shadow-2xl w-full max-w-xl max-h-[92vh] flex flex-col animate-in fade-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between p-5 border-b border-gray-100 dark:border-zinc-800 shrink-0">
              ...
              <button onClick={() => setShowCreate(false)} className="text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-300">
                <X weight="bold" className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="overflow-y-auto flex-1 p-5 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="col-span-2">
                  <label className="block text-xs font-semibold text-gray-600 dark:text-gray-300 mb-1">No. Resep *</label>
                  <input
                    type="text"
                    value={form.prescription_number}
                    onChange={e => setForm(f => ({ ...f, prescription_number: e.target.value }))}
                    placeholder="Misal: R/001/2025"
                    className="w-full border border-gray-200 dark:border-zinc-700 dark:bg-zinc-800 dark:text-gray-200 dark:placeholder-gray-500 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-600"
                  />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-gray-600 dark:text-gray-300 mb-1">Nama Pasien *</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-xs font-semibold text-gray-600 dark:text-gray-300 mb-1">Umur Pasien</label>
                  <input
                    type="number"
                    min={0}
                ...
                <div>
                  <label className="block text-xs font-semibold text-gray-600 dark:text-gray-300 mb-1">Nama Dokter *</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-xs font-semibold text-gray-600 dark:text-gray-300 mb-1">No. SIP Dokter</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-xs font-semibold text-gray-600 dark:text-gray-300 mb-1">Tanggal Resep</label>
                  <input
                    type="date"
                ...
                <div>
                  <label className="block text-xs font-semibold text-gray-600 dark:text-gray-300 mb-1">Berlaku Sampai</label>
                  <input
                    type="date"
                    ...
                    className="w-full border border-gray-200 dark:border-zinc-700 dark:bg-zinc-800 dark:text-gray-200 dark:placeholder-gray-500 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-600"
                  />
                  <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">Default: 30 hari dari tgl resep</p>
                </div>
                <div className="col-span-2">
                  <label className="block text-xs font-semibold text-gray-600 dark:text-gray-300 mb-1">Catatan Dokter</label>
                  <textarea
                    value={form.notes}
                    onChange={e => setForm(f => ({ ...f, notes: e.target.value }))}
                    placeholder="Instruksi atau catatan khusus dari dokter..."
                    rows={2}
                    className="w-full border border-gray-200 dark:border-zinc-700 dark:bg-zinc-800 dark:text-gray-200 dark:placeholder-gray-500 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-600 resize-none"
                  />
                </div>
              </div>
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  <p className="text-xs font-semibold text-gray-600 dark:text-gray-300 uppercase tracking-wide">Daftar Obat *</p>
                  <button
                ...
                <div className="space-y-2">
                  {items.map((item, idx) => (
                    <div key={idx} className="grid grid-cols-12 gap-2 items-start bg-gray-50 dark:bg-zinc-800 rounded-xl p-3">
                      <div className="col-span-5">
                        ...
                        <div className="relative">
                          <input
                            ...
                            className={`w-full border rounded-lg px-2.5 py-1.5 text-xs focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-600 ${item.medicine_id ? 'border-emerald-300 bg-emerald-50 dark:bg-emerald-950 dark:border-emerald-700' : 'border-gray-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 dark:text-gray-200 dark:placeholder-gray-500'}`}
                          />
                          {medSuggestions?.idx === idx && medSuggestions.list.length > 0 && (
                            <div className="absolute z-20 mt-0.5 w-full bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-700 rounded-xl shadow-lg overflow-hidden">
                              {medSuggestions.list.map(med => (
                                <button
                                  key={med.id}
                                  type="button"
                                  onMouseDown={() => selectMedicineSuggestion(idx, med)}
                                  className="w-full text-left px-3 py-2 text-xs hover:bg-indigo-50 dark:hover:bg-zinc-800 flex items-center justify-between border-b border-gray-50 dark:border-zinc-800 last:border-b-0"
                                >
                                  <span className="font-semibold text-gray-900 dark:text-gray-100">{med.name}</span>
                                  <span className="text-gray-400 dark:text-gray-500 ml-2 shrink-0">Stok: {med.stock} {med.unit}</span>
                                </button>
                              ))}
                            </div>
                          )}
                      ...
                      <div className="col-span-5">
                        <label className="block text-xs text-gray-400 dark:text-gray-500 mb-1">Aturan Pakai (Signa)</label>
                        <input
                          type="text"
                          value={item.signa}
                          onChange={e => updateItem(idx, 'signa', e.target.value)}
                          placeholder="3×1 sesudah makan"
                          className="w-full border border-gray-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 dark:text-gray-200 dark:placeholder-gray-500 rounded-lg px-2.5 py-1.5 text-xs focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-600"
                        />
                      </div>
                      <div className="col-span-1">
                        <label className="block text-xs text-gray-400 dark:text-gray-500 mb-1">Jml</label>
                        <input
                          type="number"
                          min={1}
                          value={item.quantity}
                          onChange={e => updateItem(idx, 'quantity', Math.max(1, Number(e.target.value)))}
                          className="w-full border border-gray-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 dark:text-gray-200 rounded-lg px-2 py-1.5 text-xs text-center focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-600"
                        />
                      </div>
                      <div className="col-span-1 flex items-end pb-0.5">
                        <button
                          ...
                          className="text-gray-300 hover:text-red-400 disabled:opacity-30 mt-4"
                        >
            ...
            <div className="flex gap-2 p-5 border-t border-gray-100 dark:border-zinc-800 shrink-0">
              ...
              <button
                onClick={handleSave}
                disabled={saving}
                className="flex-1 flex items-center justify-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white py-2.5 rounded-xl text-sm font-semibold disabled:opacity-50"
              >
