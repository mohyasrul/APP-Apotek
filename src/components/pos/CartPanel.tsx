  ...
  const panelContent = (
    <>
              ...
              return (
                <div key={item.id} className={`p-3 rounded-xl border ${isFEFO ? 'border-rose-200 bg-rose-50/20' : 'border-gray-100 bg-gray-50/50'} flex flex-col gap-2`}>
                  ...
                  {allocations[item.id] && allocations[item.id].length > 0 && (
                    <div className="mt-1 pt-2 border-t border-gray-100 dark:border-zinc-800/50 space-y-1">
                      <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider flex items-center gap-1">
                        <Tag weight="fill" className="w-2.5 h-2.5 text-indigo-400" /> Alokasi FEFO:
                      </p>
                      {allocations[item.id].map((alloc, idx) => (
                        <div key={idx} className="flex items-center justify-between text-[11px] text-gray-600 dark:text-gray-300 bg-indigo-50/50 dark:bg-indigo-900/20 px-2 py-1 rounded-md border border-indigo-100/50 dark:border-indigo-800/20">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-[9px] text-gray-400">ED: {new Date(alloc.expiry_date).toLocaleDateString('id-ID', { month: 'short', year: '2-digit' })}</span>
                          </div>
                        </div>
                      ))}
                    </div>
      ...
      <div className="p-4 bg-white border-t border-gray-100 flex-shrink-0 shadow-[0_-4px_24px_rgba(0,0,0,0.02)]">
        ...
        {globalDiscount > 0 && (
          <div className="flex items-center justify-between mb-2 text-sm">
            <span className="text-indigo-600">Diskon Total</span>
        ...
        <button
          ...
          className="w-full flex items-center justify-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-4 rounded-xl transition-all shadow-[0_4px_12px_rgba(59,130,246,0.3)] disabled:opacity-50 disabled:shadow-none"
        >
          Bayar Sekarang
